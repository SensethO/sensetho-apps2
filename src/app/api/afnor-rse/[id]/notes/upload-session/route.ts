...
/**
 * POST /api/afnor-rse/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_afnor_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[afnor-rse/notes/upload-session/counter]', counterError)
    ...
    if (!spRes.ok) {
      ...
      console.error('[afnor-rse/notes/upload-session/sp]', spRes.status, errText)

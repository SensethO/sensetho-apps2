...
/**
 * POST /api/sapin2/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_sapin2_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[sapin2/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('sapin2-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('sapin2-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[sapin2/notes/upload-session/sp]', spRes.status, errText)

 ...
 * `budget_ent_exercices_write` utilisée par les handlers.
 */
...
/**
 * POST /api/budget-entreprise/pieces/upload-session
...
export async function POST(req: NextRequest) {
  try {
    ...
    const config = await getConfigForApp('budget-entreprise')
    ...
    const spRes = await spGraphForApp('budget-entreprise', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[budget-entreprise/pieces/upload-session/sp]', spRes.status, errText)

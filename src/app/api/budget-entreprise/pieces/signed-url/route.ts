...
/**
 * GET /api/budget-entreprise/pieces/signed-url?item_id=xxx
 ...
 * Lecture : tout utilisateur authentifié (aligné sur la RLS de budget_ent_pieces).
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await spGraphForApp('budget-entreprise', `/items/${item_id}`)
    if (!res.ok) {

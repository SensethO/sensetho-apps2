...
/**
 * POST /api/afaq26000/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_afaq26000_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[afaq26000/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('afaq26000-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('afaq26000-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[afaq26000/notes/upload-session/sp]', spRes.status, errText)

...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [diagRes, repRes, actRes, notesRes] = await Promise.all([
      ...
      admin.from('vigilance_notes').select('critere_id, sections').eq('diagnostic_id', params.id),
    ])
    ...
    // ─── Onglet 5 : Notes & Annexes ───────────────────────────────────────────
    {
      ...
      sc(ws, 2, 2, 'Pièces jointes & Annexes', { bold: true, sz: 14, bg: C.red, fg: C.white })
      merge(ws, 2, 2, 2, 6)
      ws.getRow(2).height = 30
      ...
      for (const n of (notesRes.data ?? []) as any[]) {
        const axe = VIGILANCE_AXES.find(a => a.criteres.some((c: any) => c.id === n.critere_id)) as any

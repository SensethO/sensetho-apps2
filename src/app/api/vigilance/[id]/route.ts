...
/** GET /api/vigilance/[id] — détail complet */
export async function GET(
  ...
  try {
    ...
    const [diagRes, repRes, actRes] = await Promise.all([
      admin.from('vigilance_diagnostics').select('*').eq('id', params.id).single(),
...
/** PATCH /api/vigilance/[id] — mise à jour score_global et statut */

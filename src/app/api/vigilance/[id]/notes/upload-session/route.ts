...
/**
 * POST /api/vigilance/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_vigilance_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[vigilance/notes/upload-session/counter]', counterError)
    ...
    if (!spRes.ok) {
      ...
      console.error('[vigilance/notes/upload-session/sp]', spRes.status, errText)

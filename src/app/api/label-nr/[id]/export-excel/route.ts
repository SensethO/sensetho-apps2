...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [diagRes, repRes, actRes, notesRes] = await Promise.all([
      ...
      admin.from('label_nr_notes').select('critere_id, sections').eq('diagnostic_id', params.id),
    ])
    ...
    // ─── Onglet 5 : Notes & Annexes ───────────────────────────────────────────
    {
      ...
      for (const n of (notesRes.data ?? []) as any[]) {
        const axe = LABEL_NR_AXES.find(a => a.criteres.some((c: any) => c.id === n.critere_id)) as any

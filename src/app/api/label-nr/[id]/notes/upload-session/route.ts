...
/**
 * POST /api/label-nr/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_label_nr_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[label-nr/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('label-nr-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('label-nr-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[label-nr/notes/upload-session/sp]', spRes.status, errText)

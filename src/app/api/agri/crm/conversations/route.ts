...
export async function GET(req: Request) {
  try {
    ...
    if (mode === 'acheteur') {
      ...
      // 4. Derniers messages par plantation (pour cette conversation acheteur)
      const { data: allMessages } = await svc
        .from('agri_crm_messages')
        .select('plantation_id, content, created_at, lu_par, sender_user_id')
        .eq('acheteur_user_id', user.id)
      ...
      const conversations = (plantations ?? []).map((p: { id: string; nom: string; pays_nom: string; user_id: string }) => {
        const planteur = (profiles ?? []).find((pr: { id: string; email?: string; full_name?: string }) => pr.id === p.user_id) as { id: string; email?: string; full_name?: string } | undefined
    ...
    if (mode === 'planteur') {
      // 1. Toutes les plantations du planteur
      const { data: plantations } = await svc
        .from('plantations')
        .select('id, nom, pays_nom')
        .eq('user_id', user.id)

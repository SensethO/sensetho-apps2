...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_act_carbone_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[act-carbone/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('act-carbone-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('act-carbone-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[act-carbone/notes/upload-session/sp]', spRes.status, errText)

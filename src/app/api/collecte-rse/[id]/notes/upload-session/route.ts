...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_collecte_rse_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[collecte-rse/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('collecte-rse-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('collecte-rse-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[collecte-rse/notes/upload-session/sp]', spRes.status, errText)

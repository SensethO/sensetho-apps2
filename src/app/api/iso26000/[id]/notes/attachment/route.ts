...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const spRes = await spGraphForApp('iso26000', `/items/${row.sharepoint_item_id}`, {
      method: 'PATCH',
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const spRes = await spGraphForApp('iso26000', `/items/${row.sharepoint_item_id}`, { method: 'DELETE' })

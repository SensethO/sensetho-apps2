...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!await checkSubscription(user.id, 'iso26000')) {
    ...
    const { data: shared } = await admin
      .from('iso26000_diagnostics')
      .select('*, iso26000_diagnostic_shares!inner(permission, shared_with_user_id)')
      .eq('organisation_id', org_id)
      .eq('year', year)
      .eq('iso26000_diagnostic_shares.shared_with_user_id', user.id)
      .maybeSingle()

    if (shared) {
      const permission = (shared as Record<string, unknown[]>).iso26000_diagnostic_shares?.[0]

...
/**
 * POST /api/iso45001/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_iso45001_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[iso45001/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('iso45001-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('iso45001-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[iso45001/notes/upload-session/sp]', spRes.status, errText)

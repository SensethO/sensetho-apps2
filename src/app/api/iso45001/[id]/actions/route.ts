...
async function canWrite(userId: string, diagnosticId: string): Promise<boolean> {
  ...
  const { data } = await admin
    .from('iso45001_diagnostics').select('user_id').eq('id', diagnosticId).single()
  return data?.user_id === userId
}

/** GET /api/iso45001/[id]/actions?critere_id=xxx */
...
/** POST /api/iso45001/[id]/actions — créer une action */
...
/** PATCH /api/iso45001/[id]/actions?action_id=xxx */
...
/** DELETE /api/iso45001/[id]/actions?action_id=xxx */

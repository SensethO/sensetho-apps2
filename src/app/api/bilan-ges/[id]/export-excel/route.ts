...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [diagRes, repRes, actRes, notesRes] = await Promise.all([
      admin.from('bilan_ges_diagnostics').select('*, organisations(denomination, siret_siege, ville)').eq('id', params.id).single(),
      ...
      admin.from('bilan_ges_notes').select('critere_id, sections').eq('diagnostic_id', params.id),
    ])
    ...
    // ─── Onglet 5 : Notes & Annexes ───────────────────────────────────────────
    {
      ...
      for (const n of (notesRes.data ?? []) as any[]) {
        const axe = BILAN_GES_AXES.find(a => a.criteres.some((c: any) => c.id === n.critere_id)) as any

...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [diagRes, repRes, actRes] = await Promise.all([
      admin.from('iso50001_diagnostics').select('*, organisations(nom, siret, pays)').eq('id', params.id).single(),
    ...
    // ─── Onglet 5 : Notes & Annexes ───────────────────────────────────────────
    {
      ...
      sc(ws, 2, 2, 'Notes & Documents', { bold: true, sz: 14, bg: C.yellow, fg: C.white })
      merge(ws, 2, 2, 2, 3)
      ws.getRow(2).height = 30

...
/** GET /api/iso50001/[id] — détail complet */
export async function GET(
  ...
  try {
    ...
    const [diagRes, repRes, actRes] = await Promise.all([
      admin.from('iso50001_diagnostics').select('*').eq('id', params.id).single(),
...
/** PATCH /api/iso50001/[id] — mise à jour score_global et statut */

...
/**
 * POST /api/bcorp/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_bcorp_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[bcorp/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('bcorp-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('bcorp-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[bcorp/notes/upload-session/sp]', spRes.status, errText)

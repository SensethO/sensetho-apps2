...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_eudr_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[eudr/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('eudr-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('eudr-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[eudr/notes/upload-session/sp]', spRes.status, errText)

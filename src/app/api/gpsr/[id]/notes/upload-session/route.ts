...
/**
 * POST /api/gpsr/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_gpsr_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[gpsr/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('gpsr-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('gpsr-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[gpsr/notes/upload-session/sp]', spRes.status, errText)

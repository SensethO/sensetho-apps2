...
/**
 * POST /api/ecovadis/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_ecovadis_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[ecovadis/notes/upload-session/counter]', counterError)
    ...
    if (!spRes.ok) {
      ...
      console.error('[ecovadis/notes/upload-session/sp]', spRes.status, errText)

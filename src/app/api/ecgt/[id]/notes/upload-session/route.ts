...
/**
 * POST /api/ecgt/[id]/notes/upload-session
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: counterData, error: counterError } = await admin
      .rpc('increment_ecgt_notes_counter', { p_id: diagnosticId })
    if (counterError || counterData == null) {
      console.error('[ecgt/notes/upload-session/counter]', counterError)
    ...
    const config = await getConfigForApp('ecgt-diagnostic')
    const spPath = `/root:/${config.rootFolder}/${diagnosticId}/notes/${actionKey}/${finalName}:/createUploadSession`

    const spRes = await spGraphForApp('ecgt-diagnostic', spPath, {
      method: 'POST',
    ...
    if (!spRes.ok) {
      ...
      console.error('[ecgt/notes/upload-session/sp]', spRes.status, errText)

...
const LIGNE_SELECT = "*, compte:budget_ent_comptes(*), details:budget_ent_lignes_details(*)";
...
/** GET — un exercice + ses lignes (`?affectation=general|centre_cout|all`, `?centre_cout_id=`). */
export async function exerciceGET(req: NextRequest, ctx: Ctx) {
  const { id } = ctx.params;
  const { db, user } = await auth();
  if (!user) return unauth();
  const { searchParams } = req.nextUrl;
  const affectation = searchParams.get("affectation") ?? "general";
  const centreCoutId = searchParams.get("centre_cout_id");
  ...
  if (affectation !== "all") {
    ...
    if (affectation === "centre_cout" && centreCoutId)
      lignesQ = lignesQ.eq("centre_cout_id", centreCoutId);
  }

  const [{ data: exercice, error: e1 }, { data: lignes, error: e2 }] = await Promise.all([
    db.from("budget_ent_exercices").select(EXERCICE_SELECT).eq("id", id).single(),
    lignesQ,
  ]);
...
export async function exerciceDELETE(req: NextRequest, ctx: Ctx) {
  ...
  if (req.nextUrl.searchParams.get("permanent") === "true") {
    if (!(await isAdmin(user))) return forbidden();
    const { error } = await db.from("budget_ent_exercices").delete().eq("id", id);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
...
/** GET — centres de coût ayant des lignes dans l'exercice + totaux prévisionnels/réalisés. */
export async function centresCoutSubsGET(_req: NextRequest, ctx: Ctx) {
  const { id } = ctx.params;
  const { db, user } = await auth();
  if (!user) return unauth();
  ...
  const [{ data: lignes, error }, { data: allComptes }] = await Promise.all([
    db
      .from("budget_ent_lignes")
      .select("centre_cout_id, compte_id, montant_previsionnel, montant_realise, compte:budget_ent_comptes(type)")
      .eq("exercice_id", id)
      .eq("affectation_type", "centre_cout")
      .not("centre_cout_id", "is", null),
    db.from("budget_ent_comptes").select("id, parent_id"),
  ]);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  ...
  for (const l of lignes ?? []) {
    if (!l.centre_cout_id || groupIds.has(l.compte_id)) continue;
    const cur =
      byCentre.get(l.centre_cout_id) ??
    ...
    byCentre.set(l.centre_cout_id, cur);
  }
  if (byCentre.size === 0) return NextResponse.json([]);

  const { data: centres, error: e2 } = await db
    .from("budget_ent_centres_cout")
    .select("id, code, nom, statut")
    .in("id", Array.from(byCentre.keys()));
  if (e2) return NextResponse.json({ error: e2.message }, { status: 500 });
...
// ── Centres de coût (table budget_ent_centres_cout) ────────

/** GET — centres de coût (`?organisation_id=` pour filtrer), triés par nom. */
export async function centresCoutGET(req: NextRequest) {
  const { db, user } = await auth();
  if (!user) return unauth();
  const orgId = req.nextUrl.searchParams.get("organisation_id");
  let query = db.from("budget_ent_centres_cout").select("*").order("nom");
...
/** POST — créer un centre de coût `{ nom, code?, organisation_id?, statut? }`. */
export async function centresCoutPOST(req: NextRequest) {
  const { db, user } = await auth();
  if (!user) return unauth();
...
/** PATCH — mise à jour d'un centre de coût. */
export async function centreCoutPATCH(req: NextRequest, ctx: Ctx) {
  const { id } = ctx.params;
  const { db, user } = await auth();
  if (!user) return unauth();
  const b = await req.json();
  ...
  if (b.code !== undefined) fields.code = b.code ? String(b.code).trim() : null;
...
/** DELETE — supprime un centre de coût (les lignes liées gardent centre_cout_id = null). */
export async function centreCoutDELETE(_req: NextRequest, ctx: Ctx) {
  const { id } = ctx.params;
  const { db, user } = await auth();
  if (!user) return unauth();
  const { error } = await db.from("budget_ent_centres_cout").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function lignesPOST(req: NextRequest) {
  ...
  lookupQ =
    affectation_type === "centre_cout" && centre_cout_id
      ? lookupQ.eq("centre_cout_id", centre_cout_id)
      : lookupQ.is("centre_cout_id", null);
  ...
  if (existing) {
    ...
    ({ data, error } = await db
      .from("budget_ent_lignes")
      .insert({ exercice_id, compte_id, affectation_type, centre_cout_id, ...fields, updated_by: user.id })
      .select(LIGNE_SELECT)
      .single());
  }
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
 ...
 * `budget_ent_modifications` est enregistrée.
 */
export async function lignesPATCH(req: NextRequest) {
  ...
  if (champ && (old_value !== undefined || new_value !== undefined)) {
    await db.from("budget_ent_modifications").insert({
      ligne_id: id,
      champ,
...
export async function lignesDELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: "id requis" }, { status: 400 });
  const { error } = await db.from("budget_ent_lignes").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function lignesDetailsDELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: "id requis" }, { status: 400 });
  const { error } = await db.from("budget_ent_lignes_details").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  if (ligne_id) await recalcLigne(db, ligne_id);
  return NextResponse.json({ ok: true });
}
...
/**
 * POST — transfère un détail vers un autre sous-budget (général ↔ centre de coût).
...
export async function transfertDetailPOST(req: NextRequest) {
  ...
  const {
    ...
    target_centre_cout_id = null,
  } = body as {
    detail_id: string;
    from_ligne_id: string;
    exercice_id: string;
    compte_id: string;
    target_affectation_type: "general" | "centre_cout";
    target_centre_cout_id: string | null;
  };
  ...
  lookupQ =
    target_affectation_type === "centre_cout" && target_centre_cout_id
      ? lookupQ.eq("centre_cout_id", target_centre_cout_id)
      : lookupQ.is("centre_cout_id", null);
  ...
  if (!targetLigne) {
    const { data: newLigne, error: insertErr } = await db
      ...
      .insert({
        ...
        centre_cout_id: target_centre_cout_id,
        montant_previsionnel: 0,
        montant_realise: 0,
        updated_by: user.id,
      })
...
export async function importExcelPOST(req: NextRequest) {
  ...
  if (!user) return unauth();
  const body = await req.json();
  const { exercice_id, affectation_type, centre_cout_id, rows } = body as {
    exercice_id: string;
    ...
    if (existing?.id) {
      ...
      const { data: created, error: createErr } = await db
        ...
        .insert({
          ...
          compte_id: compteId,
          affectation_type,
          centre_cout_id: centre_cout_id ?? null,
          montant_previsionnel: 0,
          montant_realise: 0,
          updated_by: user.id,
        })
...
export async function piecesGET(req: NextRequest) {
  ...
  let query = db.from("budget_ent_pieces").select("*").order("created_at", { ascending: false });
...
export async function pieceDELETE(_req: NextRequest, ctx: Ctx) {
  ...
  const { db, user } = await auth();
  if (!user) return unauth();
  const { error } = await db.from("budget_ent_pieces").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  return new NextResponse(null, { status: 204 });
}
...
export async function reportsGET(req: NextRequest) {
  ...
  if (!user) return unauth();
  const { searchParams } = req.nextUrl;
  let query = db.from("budget_ent_reports").select("*").order("created_at", { ascending: false });
...
export async function reportDELETE(_req: NextRequest, ctx: Ctx) {
  ...
  const { db, user } = await auth();
  if (!user) return unauth();
  const { error } = await db.from("budget_ent_reports").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  return new NextResponse(null, { status: 204 });
}
...
export async function permissionsGET(req: NextRequest) {
  ...
  if (!user) return unauth();
  const { searchParams } = req.nextUrl;
  let query = db.from("budget_ent_permissions").select("*").order("created_at");
...
export async function permissionsPOST(req: NextRequest) {
  ...
  let lookupQ = db.from("budget_ent_permissions").select("id").eq("user_id", b.user_id);
...
export async function permissionDELETE(_req: NextRequest, ctx: Ctx) {
  ...
  if (!user) return unauth();
  if (!(await isAdmin(user))) return forbidden();
  const { error } = await db.from("budget_ent_permissions").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  return new NextResponse(null, { status: 204 });
}

...
/**
 * Sapin2PDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Loi Sapin II — Conformité Anti-Corruption » : diagnostic de
 * CONFORMITÉ — un SCORE GLOBAL /100 et des NIVEAUX (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par Sapin2DiagnosticApp (SAPIN2_AXES) */
export interface Sapin2PdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité (SAPIN2_NIVEAUX) */
export interface Sapin2PdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
/** Une action anti-corruption */
export interface Sapin2PdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface Sapin2PdfData {
  organisation: string | null
  siren: string | null
  ville: string | null
  year: number
  date: string // ex: "16 mai 2026"
  scoreLabel: string // "Score global"
  scoreValue: number // 0-100
  badge: { label: string; emoji: string; color: string }
  axes: Sapin2PdfAxe[]
  niveaux: Sapin2PdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: Sapin2PdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<Sapin2PdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<Sapin2PdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<Sapin2PdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}

/** % de conformité moyen d'un axe (moyenne des pct des critères) */
function axePct(axe: Sapin2PdfAxe, data: Sapin2PdfData): number {
...
function niveauOf(data: Sapin2PdfData, critereId: string): Sapin2PdfNiveau {
...
function CoverPage({ data }: { data: Sapin2PdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #134e4a 0%, #0f766e 48%, #115e59 100%)',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(45,212,191,0.20)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          Loi n°2016-1691 du 9 decembre 2016 — Diagnostic
        </div>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Diagnostic du programme anti-corruption (art. 17 — référentiel AFA)
        </div>
...
function OverviewPage({ data }: { data: Sapin2PdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#134e4a', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Loi Sapin II — Conformite Anti-Corruption
          </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: '#64748b', fontFamily: 'Arial, sans-serif' }}>
              Score global : {data.scoreValue}/100
            </div>
          </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Loi Sapin II · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: Sapin2PdfAxe; data: Sapin2PdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#f0fdfa', borderLeft: '3px solid #0f766e', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions anti-corruption ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (
...
export default function Sapin2PDFReport({ id, data }: { id: string; data: Sapin2PdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

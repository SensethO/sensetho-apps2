...
/**
 * Iso50001PDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Diagnostic ISO 50001 » : diagnostic de MATURITÉ — grand chiffre =
...
/** Un axe tel qu'exporté par Iso50001DiagnosticApp (ISO50001_AXES) */
export interface Iso50001PdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité (ISO50001_NIVEAUX) */
export interface Iso50001PdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
export interface Iso50001PdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface Iso50001PdfData {
  ...
  axes: Iso50001PdfAxe[]
  niveaux: Iso50001PdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: Iso50001PdfAction[]
}

// ─── Constantes d'apparence (accent ISO 50001 = jaune/or) ────────────────────

const ACCENT = '#ca8a04'
const ACCENT_DARK = '#854d0e'

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<Iso50001PdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<Iso50001PdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<Iso50001PdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}
...
function axePct(axe: Iso50001PdfAxe, data: Iso50001PdfData): number {
...
function niveauOf(data: Iso50001PdfData, critereId: string): Iso50001PdfNiveau {
...
function CoverPage({ data }: { data: Iso50001PdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: `linear-gradient(148deg, ${ACCENT_DARK} 0%, ${ACCENT} 48%, #a16207 100%)`,
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(253,224,71,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          ISO 50001:2018 — Management de l&apos;energie
        </div>
...
function OverviewPage({ data }: { data: Iso50001PdfData }) {
  const badge = data.badge
    ...
    >
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Diagnostic ISO 50001 · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: Iso50001PdfAxe; data: Iso50001PdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#fefce8', borderLeft: `3px solid ${ACCENT}`, borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
...
export default function Iso50001PDFReport({ id, data }: { id: string; data: Iso50001PdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

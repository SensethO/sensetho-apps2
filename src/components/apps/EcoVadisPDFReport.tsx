...
/**
 * EcoVadisPDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Diagnostic EcoVadis — Préparation à l’évaluation RSE » : un SCORE
 * SIMULÉ /100 (pondération des 4 thèmes EcoVadis), un badge (Bronze / Silver /
 * Gold / Platinum) et des NIVEAUX (NC / 1 / 2 / 3 / 4).
 *
 * Architecture de capture :
 *  - Pages fixes (couverture, vue d'ensemble) : data-pdf-page
 *  - Cartes par thème individuelles           : data-pdf-card
...
/** Un thème tel qu'exporté par EcoVadisDiagnosticApp (ECOVADIS_THEMES) */
export interface EcoVadisPdfTheme {
  id: string
  label: string
  icon: string
  poids: number
...
/** Un niveau de maturité (NIVEAUX) */
export interface EcoVadisPdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
/** Une action du plan d'actions */
export interface EcoVadisPdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface EcoVadisPdfData {
  organisation: string | null
  siren: string | null
  ville: string | null
  year: number
  date: string // ex: "16 mai 2026"
  scoreLabel: string // "Score simulé"
  scoreValue: number // 0-100
  badge: { label: string; emoji: string; color: string }
  themes: EcoVadisPdfTheme[]
  niveaux: EcoVadisPdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: EcoVadisPdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

/** Couleur d'accent par thème (mêmes teintes que le radar du tableau de bord) */
...
function themeColor(id: string): string {
  return THEME_HEX[id] ?? '#059669'
}

const PRIORITE_LABEL: Record<EcoVadisPdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<EcoVadisPdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<EcoVadisPdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}

/** % de maturité moyen d'un thème (moyenne des pct des critères) */
function themePct(theme: EcoVadisPdfTheme, data: EcoVadisPdfData): number {
  const total = theme.criteres.length || 1
  const sum = theme.criteres.reduce((s, c) => {
...
function niveauOf(data: EcoVadisPdfData, critereId: string): EcoVadisPdfNiveau {
...
function CoverPage({ data }: { data: EcoVadisPdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #065f46 0%, #059669 48%, #064e3b 100%)',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(52,211,153,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          EcoVadis — Preparation a l&rsquo;evaluation RSE
        </div>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Preparation a l&rsquo;evaluation RSE EcoVadis — Environnement, Social &amp; Droits Humains, Ethique des Affaires, Achats Responsables
        </div>
...
function OverviewPage({ data }: { data: EcoVadisPdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#065f46', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Diagnostic EcoVadis — Preparation a l&rsquo;evaluation RSE
          </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: '#64748b', fontFamily: 'Arial, sans-serif' }}>
              Score simule : {data.scoreValue}/100 · Seuils : 25 Bronze · 45 Silver · 65 Gold · 75 Platinum
            </div>
          </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {data.themes.map(theme => {
              const pct = themePct(theme, data)
              const color = themeColor(theme.id)
              return (
                <div key={theme.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: 18, flexShrink: 0, width: 24, textAlign: 'center' }}>{theme.icon}</span>
                  <div style={{ width: 230, flexShrink: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#1e293b', fontFamily: 'Arial, Helvetica, sans-serif' }}>{theme.label}</div>
                    <div style={{ fontSize: 9, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>Poids {Math.round(theme.poids * 100)}%</div>
                  </div>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 800, color, minWidth: 40, textAlign: 'right', fontFamily: 'Arial, Helvetica, sans-serif' }}>{pct}%</span>
                </div>
              )
            })}
          </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Diagnostic EcoVadis · {data.organisation ?? ''} · {data.year}
        </span>
...
// ─── Carte par thème (capturée séparément) ───────────────────────────────────

function ThemeCard({ theme, data, cardIndex }: { theme: EcoVadisPdfTheme; data: EcoVadisPdfData; cardIndex: number }) {
  const pct = themePct(theme, data)
  const color = themeColor(theme.id)
  const themeActions = data.actions.filter(a => theme.criteres.some(c => c.id === a.critere_id))
  ...
  return (
    ...
    >
      <div style={{
        ...
        {/* Header thème */}
        <div style={{ background: color, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
          <span style={{ fontSize: 24, flexShrink: 0 }}>{theme.icon}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 16, fontWeight: 900, color: 'white', letterSpacing: '-0.3px', fontFamily: 'Arial Black, Arial, sans-serif' }}>{theme.label}</div>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.78)', fontFamily: 'Arial, sans-serif' }}>Poids {Math.round(theme.poids * 100)}%</div>
          </div>
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#ecfdf5', borderLeft: '3px solid #059669', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {themeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Plan d&apos;actions ({themeActions.filter(a => a.statut === 'termine').length}/{themeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {themeActions.map(a => (
...
export default function EcoVadisPDFReport({ id, data }: { id: string; data: EcoVadisPdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}
    ...
    >
      ...
      <div style={{ width: 794, boxSizing: 'border-box' }}>
        {data.themes.map(theme => (
          <ThemeCard key={theme.id} theme={theme} data={data} cardIndex={cardIndex++} />
        ))}
      </div>
    </div>
  )

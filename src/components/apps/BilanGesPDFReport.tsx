...
/**
 * BilanGesPDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Bilan GES (BEGES réglementaire) » : diagnostic de MATURITÉ avec un
 * SCORE DE MATURITÉ 0-100 et des NIVEAUX de poste (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par BilanGesDiagnosticApp (BILAN_GES_AXES) */
export interface BilanGesPdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité de poste (BILAN_GES_NIVEAUX) */
export interface BilanGesPdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
/** Une action du plan de transition */
export interface BilanGesPdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface BilanGesPdfData {
  ...
  axes: BilanGesPdfAxe[]
  niveaux: BilanGesPdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: BilanGesPdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<BilanGesPdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<BilanGesPdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<BilanGesPdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}
...
function axePct(axe: BilanGesPdfAxe, data: BilanGesPdfData): number {
...
function niveauOf(data: BilanGesPdfData, critereId: string): BilanGesPdfNiveau {
...
function CoverPage({ data }: { data: BilanGesPdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #0c4a6e 0%, #0369a1 48%, #075985 100%)',
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          BEGES — Bilan des emissions de gaz a effet de serre
        </div>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Bilan d&apos;emissions de gaz a effet de serre (BEGES reglementaire)
        </div>
...
function OverviewPage({ data }: { data: BilanGesPdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#0c4a6e', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
      ...
      <div style={{ flex: 1, padding: '26px 52px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 12, fontFamily: 'Arial, sans-serif' }}>
            Niveaux de maturite des postes
          </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Bilan GES — BEGES reglementaire · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: BilanGesPdfAxe; data: BilanGesPdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#f0f9ff', borderLeft: '3px solid #0369a1', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions du plan ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (
...
export default function BilanGesPDFReport({ id, data }: { id: string; data: BilanGesPdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

...
/**
 * BcorpPDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Diagnostic B Corp » : auto-diagnostic de MATURITÉ. Le grand nombre
 * est un SCORE DE MATURITÉ /100 (et non un %). Niveaux de maturité NC / 1 / 2 / 3 / 4.
 *
...
/** Un axe tel qu'exporté par BcorpDiagnosticApp (BCORP_AXES) */
export interface BcorpPdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité (BCORP_NIVEAUX) */
export interface BcorpPdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
export interface BcorpPdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface BcorpPdfData {
  ...
  axes: BcorpPdfAxe[]
  niveaux: BcorpPdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: BcorpPdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<BcorpPdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<BcorpPdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<BcorpPdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}
...
function axePct(axe: BcorpPdfAxe, data: BcorpPdfData): number {
...
function niveauOf(data: BcorpPdfData, critereId: string): BcorpPdfNiveau {
...
function CoverPage({ data }: { data: BcorpPdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #92400e 0%, #b45309 48%, #78350f 100%)',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(251,191,36,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          B Impact Assessment — Auto-diagnostic
        </div>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Auto-diagnostic de maturite B Corp (B Impact Assessment)
        </div>
...
function OverviewPage({ data }: { data: BcorpPdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#92400e', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Diagnostic B Corp — B Impact Assessment
          </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Diagnostic B Corp — B Impact Assessment · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: BcorpPdfAxe; data: BcorpPdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#fffbeb', borderLeft: '3px solid #b45309', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
...
export default function BcorpPDFReport({ id, data }: { id: string; data: BcorpPdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

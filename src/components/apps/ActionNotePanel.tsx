...
function DownloadAllButton({
  attachments, sessionId, actionKey, apiBase,
}: {
  attachments: AttachmentMeta[]
  sessionId: string
  actionKey: string
    ...
    try {
      for (const att of attachments) {
        try {
          ...
          setDone(d => d + 1)
          // Petit délai pour éviter que le navigateur bloque les téléchargements simultanés
...
function AttachmentItem({
  ...
  useEffect(() => {
    if (!isImage) return
    let cancelled = false
    getSignedUrl(apiBase, sessionId, actionKey, att.path)
  ...
  async function openViewer() {
    setLoading('open')
    try { setViewerUrl(await getSignedUrl(apiBase, sessionId, actionKey, att.path)) }
  ...
  async function confirmRename() {
    ...
    setRenaming(true)
    try {
      const res = await fetch(`${apiBase}/${sessionId}/annexes`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ attachmentId: att.id, fileName: newName }),
      })
      if (!res.ok) {
        const j = await res.json().catch(() => ({}))
        alert(j.error ?? 'Erreur lors du renommage')
        return
      }
      onRename?.(newName)
      setEditing(false)
...
function SectionEditor({
  ...
  async function handleFileSelect(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const { uploadUrl, attachmentId, finalName, confirmUrl } = sessionJson as {
        ...
        if (confirmUrl) {
          try {
            ...
            const confirmJson = await confirmRes.json() as { folderName?: string }
            folderName = confirmJson.folderName
          } catch { /* non-bloquant */ }
        }
  ...
  async function deleteAttachment(att: AttachmentMeta) {
    try {
      const encodedKey = encodeURIComponent(actionKey)
      await fetch(
        `${apiBase}/${sessionId}/action-notes/${encodedKey}/attachments/${att.id}`,
        { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path: att.path }) }
      )
      onChange({
        ...sectionRef.current,
...
interface ActionNotePanelProps {
  sessionId: string
  actionKey: string
  /** Pre-loaded initial sections (pass [] if unknown). The panel will fetch from API if null. */
  initialSections: NoteSection[] | null
  onSaved?: (sections: NoteSection[]) => void
  readOnly?: boolean
  /** API base path without trailing slash. Default: /api/iso26000/sessions */
  apiBase?: string
  /** Supabase table name for Realtime subscription. Default: iso26000_action_notes */
  realtimeTable?: string
  /** key = attachment_id → stable annexe ref like "A01" */
  annexeRefs?: Map<string, string>
  /** Called after a new file is uploaded so parent can reload annexe refs.
   *  Receives the SharePoint folder name (to update session card info button). */
  onAttachmentUploaded?: (folderName?: string) => void
  /** Données supplémentaires envoyées avec chaque upload (ex: plantation_id) */
  uploadExtraFormData?: Record<string, string>
}
...
export default function ActionNotePanel({
  sessionId,
  actionKey,
  initialSections,
  onSaved,
  readOnly = false,
  apiBase = '/api/iso26000/sessions',
  realtimeTable = 'iso26000_action_notes',
  annexeRefs,
  onAttachmentUploaded,
  uploadExtraFormData,
}: ActionNotePanelProps) {
  const [sections, setSections] = useState<NoteSection[]>(() =>
    initialSections ?? []
  )
  const [loading,   setLoading]   = useState(initialSections === null)
  ...
  // Load from API if initialSections is null
  useEffect(() => {
    ...
    fetch(`${apiBase}/${sessionId}/action-notes`)
      .then(r => r.ok ? r.json() : null)
      .then(j => {
        const loaded = j?.data?.[actionKey] ?? []
        setSections(loaded.length > 0 ? loaded : [newSection()])
      })
      .catch(() => setSections([newSection()]))
      .finally(() => setLoading(false))
  }, [sessionId, actionKey]) // eslint-disable-line react-hooks/exhaustive-deps

  // When initialSections changes from parent (e.g. parent reloads notes list)
  useEffect(() => {
    if (initialSections !== null && initialSections.length > 0 && !pendingSaveRef.current) {
  ...
  const scheduleSave = useCallback((newSections: NoteSection[]) => {
    if (readOnly) return
    pendingSaveRef.current = true        // block remote sync
    ...
    saveTimerRef.current = setTimeout(async () => {
      try {
        const encodedKey = encodeURIComponent(actionKey)
        const res = await fetch(
          `${apiBase}/${sessionId}/action-notes/${encodedKey}`,
          {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        pendingSaveRef.current = false   // allow remote sync again
  ...
  if (loading) {
    return (
      <div className="mt-2 p-4 rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <p className="text-xs text-gray-400 dark:text-gray-300 text-center">Chargement…</p>
      </div>
    )
  }
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className={`text-xs ${
            ...
            saveStatus === 'synced' ? 'text-blue-400' :
            saveStatus === 'error'  ? 'text-red-500' : 'text-transparent'
          }`}>
            {saveStatus === 'saving' ? '● Enregistrement…' :
             saveStatus === 'saved'  ? '✓ Enregistré' :
             saveStatus === 'synced' ? '⟳ Synchronisé' :

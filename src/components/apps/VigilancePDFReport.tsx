...
/**
 * VigilancePDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Plan de vigilance » : diagnostic de MATURITÉ — grand chiffre =
 * Score de maturité /100, niveaux de maturité (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par VigilanceDiagnosticApp (VIGILANCE_AXES) */
export interface VigilancePdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité (VIGILANCE_NIVEAUX) */
export interface VigilancePdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
export interface VigilancePdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface VigilancePdfData {
  ...
  axes: VigilancePdfAxe[]
  niveaux: VigilancePdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: VigilancePdfAction[]
}

// ─── Constantes d'apparence (accent Vigilance = rouge) ───────────────────────
...
const PRIORITE_LABEL: Record<VigilancePdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<VigilancePdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<VigilancePdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}

/** Score de maturité moyen d'un axe (moyenne des pct des critères) */
function axePct(axe: VigilancePdfAxe, data: VigilancePdfData): number {
...
function niveauOf(data: VigilancePdfData, critereId: string): VigilancePdfNiveau {
...
function CoverPage({ data }: { data: VigilancePdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: `linear-gradient(148deg, ${ACCENT_DARK} 0%, ${ACCENT} 48%, #b91c1c 100%)`,
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(254,202,202,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        <div style={{ fontSize: 76, marginBottom: 24, lineHeight: 1 }}>🛡️</div>
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          Loi n&deg;2017-399 — Devoir de vigilance
        </div>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.65)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Evaluation de la maturite du plan de vigilance et de la prevention des atteintes
        </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.09)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 18, padding: '22px 48px', marginBottom: 44, minWidth: 360 }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', fontFamily: 'Arial, sans-serif' }}>
          ...
          <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', fontFamily: 'Arial, sans-serif' }}>
            Annee {data.year}
          </div>
        </div>

        {/* Grand score /100 + badge */}
        <div style={{ fontSize: 88, fontWeight: 900, lineHeight: 1, fontFamily: 'Arial Black, Arial, sans-serif' }}>
          {data.scoreValue}
          <span style={{ fontSize: 36, fontWeight: 700, opacity: 0.7 }}>/100</span>
        </div>
        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', marginTop: 4, marginBottom: 16, fontFamily: 'Arial, sans-serif', letterSpacing: 1 }}>
      ...
      <div style={{ padding: '16px 52px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', fontFamily: 'Arial, sans-serif' }}>
          Genere avec Sens&apos;ethO Apps · apps.sensetho.com
        </span>
        <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', fontFamily: 'Arial, sans-serif' }}>Page 1</span>
      </div>
    </div>
  )
}
...
function OverviewPage({ data }: { data: VigilancePdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: ACCENT_DARK, padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Devoir de vigilance — Loi n&deg;2017-399
          </div>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', fontFamily: 'Arial, sans-serif' }}>
            {data.scoreLabel}
          </div>
        </div>
      </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: '#64748b', fontFamily: 'Arial, sans-serif' }}>
              {data.scoreLabel} : {data.scoreValue}/100
            </div>
          </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Plan de vigilance · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: VigilancePdfAxe; data: VigilancePdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#fef2f2', borderLeft: `3px solid ${ACCENT}`, borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (
...
export default function VigilancePDFReport({ id, data }: { id: string; data: VigilancePdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

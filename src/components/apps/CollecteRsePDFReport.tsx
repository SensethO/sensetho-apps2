...
/**
 * CollecteRsePDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Collecte documentaire RSE » : pas de score /5, mais un TAUX DE
 * COMPLÉTUDE 0-100 % et des ÉTATS de document (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par CollecteRseDiagnosticApp (COLLECTE_RSE_AXES) */
export interface CollectePdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau / état de document (COLLECTE_RSE_NIVEAUX) */
export interface CollectePdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
/** Une action de collecte */
export interface CollectePdfAction {
  id: string
  critere_id: string
  titre: string
...
const PRIORITE_LABEL: Record<CollectePdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<CollectePdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<CollectePdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}

/** Badge de complétude (mêmes seuils que CollecteRseDiagnosticApp) */
function getBadge(score: number): { label: string; color: string; icon: string } {
  if (score >= 85) return { label: 'Dossier exemplaire', color: '#4f46e5', icon: '⭐' }
  if (score >= 60) return { label: 'Dossier prêt pour le diagnostic', color: '#16a34a', icon: '✅' }
  if (score >= 30) return { label: 'Collecte en cours', color: '#f97316', icon: '🔄' }
  return { label: 'Dossier insuffisant', color: '#dc2626', icon: '❌' }
}

/** % de complétude moyen d'un axe (moyenne des pct des 4 critères) */
function axePct(axe: CollectePdfAxe, data: CollectePdfData): number {
...
function niveauOf(data: CollectePdfData, critereId: string): CollectePdfNiveau {
...
function CoverPage({ data }: { data: CollectePdfData }) {
  const badge = getBadge(data.completude)
  return (
    <div
      data-pdf-page="0-cover"
      style={{
        width: 794, height: 1123,
        background: 'linear-gradient(148deg, #312e81 0%, #4f46e5 48%, #4338ca 100%)',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(129,140,248,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          ISO 26000 — Preparation au diagnostic
        </div>
...
function OverviewPage({ data }: { data: CollectePdfData }) {
  const badge = getBadge(data.completude)
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#312e81', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Collecte documentaire RSE — ISO 26000
          </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: '#64748b', fontFamily: 'Arial, sans-serif' }}>
              Taux de completude du dossier : {data.completude}%
            </div>
          </div>
          <div style={{ width: 180, height: 9, background: '#e2e8f0', borderRadius: 5, overflow: 'hidden', flexShrink: 0 }}>
            <div style={{ width: `${data.completude}%`, height: '100%', background: badge.color, borderRadius: 5 }} />
          </div>
        </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Collecte documentaire RSE — ISO 26000 · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: CollectePdfAxe; data: CollectePdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
      ...
      <div style={{
        ...
        <div style={{ background: axe.color, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.7)', fontFamily: 'Arial, sans-serif' }}>completude</div>
          </div>
        </div>
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#f5f3ff', borderLeft: '3px solid #6366f1', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions de collecte ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (

...
/**
 * LabelNrPDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Label Numérique Responsable » : diagnostic de MATURITÉ — un SCORE
 * de maturité 0-100 (et non un taux) et des NIVEAUX de maturité (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par LabelNrDiagnosticApp (LABEL_NR_AXES) */
export interface LabelNrPdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité (LABEL_NR_NIVEAUX) */
export interface LabelNrPdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
/** Une action NR */
export interface LabelNrPdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface LabelNrPdfData {
  ...
  axes: LabelNrPdfAxe[]
  niveaux: LabelNrPdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: LabelNrPdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<LabelNrPdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<LabelNrPdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<LabelNrPdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#0d9488',
}

/** Maturité moyenne d'un axe (moyenne des pct des critères) */
function axePct(axe: LabelNrPdfAxe, data: LabelNrPdfData): number {
...
function niveauOf(data: LabelNrPdfData, critereId: string): LabelNrPdfNiveau {
...
function CoverPage({ data }: { data: LabelNrPdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #0f766e 0%, #0d9488 48%, #115e59 100%)',
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          Numerique Responsable — Diagnostic de maturite
        </div>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 540 }}>
          Diagnostic de maturite Numerique Responsable (Label NR — Agence LUCIE / INR)
        </div>
...
function OverviewPage({ data }: { data: LabelNrPdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#0f766e', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Label Numerique Responsable — Label NR
          </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: '#64748b', fontFamily: 'Arial, sans-serif' }}>
              Score de maturite Numerique Responsable : {data.scoreValue}/100
            </div>
          </div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12 }}>
            {[
              ...
              { label: 'Terminees', value: terminees, color: '#0d9488' },
            ].map(s => (
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Label Numerique Responsable — Label NR · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: LabelNrPdfAxe; data: LabelNrPdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions de l&apos;app ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (
...
export default function LabelNrPDFReport({ id, data }: { id: string; data: LabelNrPdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

...
/**
 * ActCarbonePDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Diagnostic ACT Bas-Carbone » : SCORE DE MATURITÉ 0-100 et des
 * NIVEAUX de maturité climatique (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par ActCarboneDiagnosticApp (ACT_CARBONE_AXES) */
export interface ActCarbonePdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité climatique (ACT_CARBONE_NIVEAUX) */
export interface ActCarbonePdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
export interface ActCarbonePdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface ActCarbonePdfData {
  ...
  axes: ActCarbonePdfAxe[]
  niveaux: ActCarbonePdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: ActCarbonePdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<ActCarbonePdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<ActCarbonePdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<ActCarbonePdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}
...
function axePct(axe: ActCarbonePdfAxe, data: ActCarbonePdfData): number {
...
function niveauOf(data: ActCarbonePdfData, critereId: string): ActCarbonePdfNiveau {
...
function CoverPage({ data }: { data: ActCarbonePdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #14532d 0%, #16a34a 48%, #15803d 100%)',
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Assessing low-Carbon Transition — Évaluation de la transition bas-carbone
        </div>
...
function OverviewPage({ data }: { data: ActCarbonePdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      ...
      <div style={{ background: '#14532d', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Diagnostic ACT — Transition bas-carbone
          </div>
      ...
      <div style={{ flex: 1, padding: '26px 52px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: '#64748b', fontFamily: 'Arial, sans-serif' }}>
              Score de maturité : {data.scoreValue}/100
            </div>
          </div>
        ...
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 12, fontFamily: 'Arial, sans-serif' }}>
            Niveaux de maturité climatique
          </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Diagnostic ACT — Transition bas-carbone · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: ActCarbonePdfAxe; data: ActCarbonePdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
      ...
      <div style={{
        ...
        <div style={{ background: axe.color, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.7)', fontFamily: 'Arial, sans-serif' }}>maturité</div>
          </div>
        </div>
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#f0fdf4', borderLeft: '3px solid #16a34a', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions de transition ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (
...
export default function ActCarbonePDFReport({ id, data }: { id: string; data: ActCarbonePdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

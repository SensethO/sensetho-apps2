...
/**
 * GpsrPDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Diagnostic GPSR » : diagnostic de MATURITÉ avec un SCORE 0-100
 * (et non un taux de complétude). Le score, son libellé et le badge sont
 * passés en props (scoreValue / scoreLabel / badge).
 *
...
/** Un axe tel qu'exporté par GpsrDiagnosticApp (GPSR_AXES) */
export interface GpsrPdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité GPSR (GPSR_NIVEAUX) */
export interface GpsrPdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
/** Une action de mise en conformité */
export interface GpsrPdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface GpsrPdfData {
  organisation: string | null
  siren: string | null
  ville: string | null
  year: number
  date: string // ex: "16 mai 2026"
  scoreLabel: string // ex: "Score de maturité"
  scoreValue: number // 0-100
  badge: { label: string; icon: string; color: string }
  axes: GpsrPdfAxe[]
  niveaux: GpsrPdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: GpsrPdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<GpsrPdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<GpsrPdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<GpsrPdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}
...
function axePct(axe: GpsrPdfAxe, data: GpsrPdfData): number {
...
function niveauOf(data: GpsrPdfData, critereId: string): GpsrPdfNiveau {
...
function CoverPage({ data }: { data: GpsrPdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #075985 0%, #0284c7 48%, #0369a1 100%)',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(125,211,252,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          GPSR — Securite generale des produits
        </div>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Reglement (UE) 2023/988 — securite generale des produits (GPSR)
        </div>
...
function OverviewPage({ data }: { data: GpsrPdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#075985', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Diagnostic GPSR — Reglement (UE) 2023/988
          </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Diagnostic GPSR — Reglement (UE) 2023/988 · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: GpsrPdfAxe; data: GpsrPdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#f0f9ff', borderLeft: '3px solid #0284c7', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions de mise en conformite ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (
...
export default function GpsrPDFReport({ id, data }: { id: string; data: GpsrPdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

...
/**
 * AfnorRsePDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Label Engagé RSE — AFNOR Certification » : diagnostic de MATURITÉ RSE —
 * un SCORE DE MATURITÉ /100 et des NIVEAUX (NC / ⭐ / ⭐⭐ / ⭐⭐⭐ / ⭐⭐⭐⭐).
 *
...
/** Un axe tel qu'exporté par AfnorRseDiagnosticApp (AFNOR_AXES) */
export interface AfnorRsePdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité (AFNOR_NIVEAUX) */
export interface AfnorRsePdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
/** Une action d'amélioration RSE */
export interface AfnorRsePdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface AfnorRsePdfData {
  organisation: string | null
  siren: string | null
  ville: string | null
  year: number
  date: string // ex: "16 mai 2026"
  scoreLabel: string // "Score de maturité RSE"
  scoreValue: number // 0-100
  badge: { label: string; emoji: string; color: string }
  axes: AfnorRsePdfAxe[]
  niveaux: AfnorRsePdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: AfnorRsePdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<AfnorRsePdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<AfnorRsePdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<AfnorRsePdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}
...
function axePct(axe: AfnorRsePdfAxe, data: AfnorRsePdfData): number {
...
function niveauOf(data: AfnorRsePdfData, critereId: string): AfnorRsePdfNiveau {
...
function CoverPage({ data }: { data: AfnorRsePdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #4c1d95 0%, #7c3aed 48%, #2e1065 100%)',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(167,139,250,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          Responsabilite Societale — Diagnostic
        </div>
        <div style={{ fontSize: 40, fontWeight: 900, margin: '0 0 8px', lineHeight: 1.1, letterSpacing: '-1.5px', fontFamily: 'Arial Black, Arial, sans-serif' }}>
          Label Engagé RSE — AFNOR Certification
        </div>
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Certification française de maturité RSE fondée sur ISO 26000
        </div>
...
function OverviewPage({ data }: { data: AfnorRsePdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#4c1d95', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: '#64748b', fontFamily: 'Arial, sans-serif' }}>
              Score de maturite RSE : {data.scoreValue}/100 · Seuils : 25% Engagé · 50% Confirmé · 75% Avancé · 90% Exemplaire
            </div>
          </div>
            ...
            {data.niveaux.map(n => (
              <div key={n.value} style={{
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 10, fontWeight: 900, color: n.color, fontFamily: 'Arial Black, Arial, sans-serif', whiteSpace: 'nowrap' }}>{n.shortLabel}</span>
                </div>
                <div style={{ flex: 1 }}>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Label Engagé RSE · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: AfnorRsePdfAxe; data: AfnorRsePdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
      ...
      <div style={{
        ...
        <div style={{ background: axe.color, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.7)', fontFamily: 'Arial, sans-serif' }}>maturite RSE</div>
          </div>
        </div>
            ...
            return (
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  {/* Badge niveau */}
                  <div style={{
                    minWidth: 26, height: 26, borderRadius: 6, flexShrink: 0, marginTop: 1, padding: '0 5px',
                    background: `${niv.color}1a`, border: `1.5px solid ${niv.color}`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>
                    <span style={{ fontSize: 9, fontWeight: 900, color: niv.color, fontFamily: 'Arial, sans-serif', whiteSpace: 'nowrap' }}>{niv.shortLabel}</span>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#f5f3ff', borderLeft: '3px solid #7c3aed', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions d’amelioration ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (
...
export default function AfnorRsePDFReport({ id, data }: { id: string; data: AfnorRsePdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

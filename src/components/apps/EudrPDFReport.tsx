...
/**
 * EudrPDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Diagnostic EUDR » : diagnostic de conformité/MATURITÉ — grand chiffre =
 * Score de conformité /100, niveaux (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par EudrDiagnosticApp (EUDR_AXES) */
export interface EudrPdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de conformité (EUDR_NIVEAUX) */
export interface EudrPdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
export interface EudrPdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface EudrPdfData {
  ...
  axes: EudrPdfAxe[]
  niveaux: EudrPdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: EudrPdfAction[]
}

// ─── Constantes d'apparence (accent EUDR = vert) ─────────────────────────────
...
const PRIORITE_LABEL: Record<EudrPdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<EudrPdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<EudrPdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}

/** Score de conformité moyen d'un axe (moyenne des pct des critères) */
function axePct(axe: EudrPdfAxe, data: EudrPdfData): number {
...
function niveauOf(data: EudrPdfData, critereId: string): EudrPdfNiveau {
...
function CoverPage({ data }: { data: EudrPdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: `linear-gradient(148deg, ${ACCENT_DARK} 0%, ${ACCENT} 48%, #15803d 100%)`,
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(134,239,172,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          Reglement (UE) 2023/1115 — Deforestation
        </div>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.65)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Evaluation de la conformite a la diligence raisonnee sans deforestation
        </div>
...
function OverviewPage({ data }: { data: EudrPdfData }) {
  const badge = data.badge
    ...
    >
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Diagnostic EUDR · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: EudrPdfAxe; data: EudrPdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
      ...
      <div style={{
        ...
        <div style={{ background: axe.color, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.7)', fontFamily: 'Arial, sans-serif' }}>conformite</div>
          </div>
        </div>
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#f0fdf4', borderLeft: `3px solid ${ACCENT}`, borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
...
export default function EudrPDFReport({ id, data }: { id: string; data: EudrPdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

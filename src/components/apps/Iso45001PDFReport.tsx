...
/**
 * Iso45001PDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Diagnostic ISO 45001 » : diagnostic de MATURITÉ — un SCORE DE
 * MATURITÉ /100 et des NIVEAUX (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par Iso45001DiagnosticApp (ISO45001_AXES) */
export interface Iso45001PdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité (ISO45001_NIVEAUX) */
export interface Iso45001PdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
/** Une action SST */
export interface Iso45001PdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface Iso45001PdfData {
  ...
  axes: Iso45001PdfAxe[]
  niveaux: Iso45001PdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: Iso45001PdfAction[]
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const PRIORITE_LABEL: Record<Iso45001PdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<Iso45001PdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<Iso45001PdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}
...
function axePct(axe: Iso45001PdfAxe, data: Iso45001PdfData): number {
...
function niveauOf(data: Iso45001PdfData, critereId: string): Iso45001PdfNiveau {
...
function CoverPage({ data }: { data: Iso45001PdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, #9a3412 0%, #ea580c 48%, #7c2d12 100%)',
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(251,146,60,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 4, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', marginBottom: 10, fontFamily: 'Arial, sans-serif' }}>
          Sante &amp; Securite au Travail — Diagnostic
        </div>
...
function OverviewPage({ data }: { data: Iso45001PdfData }) {
  const badge = data.badge
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ background: '#9a3412', padding: '18px 52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 3, fontFamily: 'Arial, sans-serif' }}>
            Diagnostic ISO 45001 — Sante &amp; Securite au Travail
          </div>
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Diagnostic ISO 45001 · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: Iso45001PdfAxe; data: Iso45001PdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#fff7ed', borderLeft: '3px solid #ea580c', borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
        ...
        {axeActions.length > 0 && (
          <div style={{ padding: '8px 16px 12px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8, fontFamily: 'Arial, sans-serif' }}>
              Actions SST ({axeActions.filter(a => a.statut === 'termine').length}/{axeActions.length})
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {axeActions.map(a => (
...
export default function Iso45001PDFReport({ id, data }: { id: string; data: Iso45001PdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}

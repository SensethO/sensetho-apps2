...
/**
 * Afaq26000PDFReport — composant visuel pour export html2canvas → jsPDF
 * (moteur générique : src/lib/pdf/exportReport.ts).
 *
 * Modèle « Évaluation AFAQ 26000 » : diagnostic de MATURITÉ — grand chiffre =
 * Score de maturité /100, niveaux d'évaluation (NC / 1 / 2 / 3 / 4).
 *
...
/** Un axe tel qu'exporté par Afaq26000DiagnosticApp (AFAQ26000_AXES) */
export interface Afaq26000PdfAxe {
  id: string
  label: string
  icon: string
  color: string
...
/** Un niveau de maturité (AFAQ26000_NIVEAUX) */
export interface Afaq26000PdfNiveau {
  value: number
  shortLabel: string
  label: string
  description: string
...
export interface Afaq26000PdfAction {
  id: string
  critere_id: string
  titre: string
...
export interface Afaq26000PdfData {
  ...
  axes: Afaq26000PdfAxe[]
  niveaux: Afaq26000PdfNiveau[]
  /** critere_id → niveau (0-4) */
  reponses: Record<string, number>
  /** critere_id → commentaire */
  commentaires: Record<string, string>
  actions: Afaq26000PdfAction[]
}

// ─── Constantes d'apparence (accent AFAQ 26000 = violet) ─────────────────────
...
const PRIORITE_LABEL: Record<Afaq26000PdfAction['priorite'], string> = {
  haute: '🔴 Haute',
  moyenne: '🟡 Moyenne',
  basse: '🟢 Basse',
}
const STATUT_LABEL: Record<Afaq26000PdfAction['statut'], string> = {
  a_faire: 'À faire',
  en_cours: 'En cours',
  termine: 'Terminé',
}
const STATUT_COLOR: Record<Afaq26000PdfAction['statut'], string> = {
  a_faire: '#64748b',
  en_cours: '#2563eb',
  termine: '#16a34a',
}
...
function axePct(axe: Afaq26000PdfAxe, data: Afaq26000PdfData): number {
...
function niveauOf(data: Afaq26000PdfData, critereId: string): Afaq26000PdfNiveau {
...
function CoverPage({ data }: { data: Afaq26000PdfData }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: `linear-gradient(148deg, ${ACCENT_DARK} 0%, ${ACCENT} 48%, #6d28d9 100%)`,
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 280, right: 80, width: 130, height: 130, borderRadius: '50%', background: 'rgba(196,181,253,0.22)' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 52px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.65)', marginBottom: 46, fontStyle: 'italic', fontFamily: 'Georgia, serif', maxWidth: 520 }}>
          Evaluation de la maturite RSE selon la grille AFAQ 26000 d&apos;AFNOR Certification
        </div>
...
function OverviewPage({ data }: { data: Afaq26000PdfData }) {
  const badge = data.badge
    ...
    >
      ...
      <div style={{ padding: '12px 52px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'Arial, sans-serif' }}>
          Evaluation AFAQ 26000 · {data.organisation ?? ''} · {data.year}
        </span>
...
function AxeCard({ axe, data, cardIndex }: { axe: Afaq26000PdfAxe; data: Afaq26000PdfData; cardIndex: number }) {
  const pct = axePct(axe, data)
              ...
              <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment && (
                      <div style={{ marginTop: 5, background: '#f5f3ff', borderLeft: `3px solid ${ACCENT}`, borderRadius: '0 6px 6px 0', padding: '5px 9px' }}>
...
export default function Afaq26000PDFReport({ id, data }: { id: string; data: Afaq26000PdfData }) {
  let cardIndex = 0
  return (
    <div
      id={id}
